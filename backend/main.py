...
def _build_formats(info):
    ...
    for f in info.get("formats", []):
        ...
        if v != "none" and h:
            ...
            if l not in seen:
                seen.add(l)
                fmts.append({"format_id": f.get("format_id"),"label":l,"ext":f.get("ext",""),"type":"video","height":h,"filesize":f.get("filesize") or f.get("filesize_approx")})
    fmts.sort(key=lambda x: (x.get("height") or 0),reverse=True)
    fmts.insert(0,{"format_id":"bestvideo+bestaudio/best","label":"Best Quality","ext":"mp4","type":"video","height":None,"filesize":None})
    return fmts
...
def get_info(body: InfoRequest):
    ...
    opts = _apply_cookies({"quiet":True,"no_warnings":True,"skip_download":True}, load_settings())
    try:
        with yt_dlp.YoutubeDL(opts) as y: info = y.extract_info(body.url, download=False)
    except Exception as e: raise HTTPException(422, detail=str(e))
    if not info: raise HTTPException(422, detail="Could not fetch video info")
    return {"title":info.get("title","Unknown"),"thumbnail":info.get("thumbnail"),"duration":info.get("duration"),"uploader":info.get("uploader"),"platform":info.get("extractor_key","").lower(),"formats":_build_formats(info),"subtitles":list(info.get("subtitles",{}).keys()),"auto_captions":list(info.get("automatic_captions",{}).keys())}

@app.post("/api/playlist-info")
def get_playlist_info(body: InfoRequest):
    if not validate_url(body.url): raise HTTPException(400, detail="Invalid URL")
    opts = _apply_cookies({"quiet":True,"no_warnings":True,"skip_download":True,"extract_flat":True}, load_settings())
    try:
        with yt_dlp.YoutubeDL(opts) as y: info = y.extract_info(body.url, download=False)
    except Exception as e: raise HTTPException(422, detail=str(e))
    if not info: raise HTTPException(422, detail="Could not fetch playlist info")
    entries = info.get("entries",[])
    return {"title":info.get("title","Playlist"),"count":len(entries),"entries":[{"id":e.get("id"),"title":e.get("title",f"Video {i+1}"),"url":e.get("url") or e.get("webpage_url"),"duration":e.get("duration"),"thumbnail":e.get("thumbnail")} for i,e in enumerate(entries)]}
...
async def download(body: DownloadRequest):
    ...
    if body.audio_only:
        fmt = "bestaudio/best"
        pp = [{"key":"FFmpegExtractAudio","preferredcodec":body.audio_format or settings.get("default_audio_format","mp3"),"preferredquality":body.audio_quality or settings.get("default_audio_quality","0")}]
    else:
    ...
    if body.subtitles: pp.append({"key":"FFmpegSubtitlesConvertor","format":"srt"})
    opts = {"format":fmt,"outtmpl":out_tmpl,"quiet":True,"no_warnings":True,"postprocessors":pp,"writethumbnail":body.thumbnail}
    if body.subtitles:
        lang = body.subtitle_lang or settings.get("subtitle_lang","en")
        opts.update({"writesubtitles":True,"writeautomaticsub":True,"subtitleslangs":[lang_]})
    rate = _parse_ratelimit(settings.get("speed_limit",""))
    if rate: opts["ratelimit"] = rate
    opts = _apply_cookies(opts, settings)
    async def stream():
        ...
        class PH:
            def __call__(s,d):
                st = d.get("status")
                if st == "downloading":
                    t=(d.get("total_bytes") or d.get("total_bytes_estimate",0)); dl=d.get("downloaded_bytes",0)
                    loop.call_soon_threadsafe(q.put_nowait,json.dumps({"type":"progress","percent":round(dl/t*100 if t else 0,1),"speed":int(d.get("speed") or 0),"eta":int(d.get("eta") or 0),"downloaded":dl,"total":t}))
                elif st == "finished":
        ...
        def run():
            try:
                ...
                errs.append(str(e)); loop.call_soon_threadsafe(q.put_nowait,json.dumps({"type":"error","message":str(e)}))
            finally: loop.call_soon_threadsafe(q.put_nowait,"__done__")
        ...
        if not errs:
            ...
            if f:
                st = f.stat()
                append_history({"id":job_id,"url":body.url,"title":body.title or f.stem,"thumbnail_url":body.thumbnail_url,"platform":body.platform or "unknown","quality_label":body.quality_label or("audio" if body.audio_only else "video"),"filename":f.name,"filesize":st.st_size,"downloaded_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"audio_only":body.audio_only})
                yield f"data: {json.dumps({'type':'complete','id':job_id,'filename':f.name,'filesize':st.st_size})}\n\n"
            else: yield f"data: {json.dumps({'type':'error', 'message':'Output file not found'})}\n\n"
